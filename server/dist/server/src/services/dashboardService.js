...
exports.dashboardService = {
    ...
    async getDashboardSummary(startDate, endDate, stores) {
        ...
        const params = [startWeekIso, endWeekIso];
        if (stores?.length) {
    ...
    async getTimeSeries(startDate, endDate, stores) {
        // Convert dates to week_iso format for direct filtering
        const startYear = new Date(startDate).getFullYear();
        const endYear = new Date(endDate).getFullYear();
        const startMonth = new Date(startDate).getMonth() + 1;
    ...
    async getTimeSeriesByStore(startDate, endDate, stores) {
        // Convert dates to week_iso format for direct filtering
        const startYear = new Date(startDate).getFullYear();
        const endYear = new Date(endDate).getFullYear();
        const startMonth = new Date(startDate).getMonth() + 1;
    ...
    async getStorePerformance(startDate, endDate) {
      ...
      ORDER BY sales_rank
    `;
        const result = await database_1.pool.query(query, [startYear, endYear]);
        return result.rows.map(row => ({
    ...
    async getDetailedWeeklyData(startDate, endDate, stores) {
        let query = `
      SELECT 
        pwd.*,
        ps.store_name,
