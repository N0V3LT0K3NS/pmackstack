...
exports.renojaService = void 0;
const database_1 = require("../config/database");
exports.renojaService = {
    async getDashboardSummary(storeFilter, startDate, endDate) {
    ...
    async getTimeSeries(storeFilter, startDate, endDate, groupBy = 'week') {
        const end = endDate || new Date().toISOString().split('T')[0];
    ...
    async submitWeeklyEntry(entryData, userId) {
        ...
        try {
            ...
            const existingCheck = await client.query('SELECT id FROM renoja_weekly_metrics WHERE store_code = $1 AND fiscal_year = $2 AND week_number = $3', [entryData.storeCode, entryData.fiscalYear, entryData.weekNumber]);
            if (existingCheck.rows.length > 0) {
                throw new Error('Entry for this store and week already exists');
            }
            // Calculate derived fields
            const previousWeekResult = await client.query(`SELECT total_paying_members 
         FROM renoja_weekly_metrics 
         WHERE store_code = $1 AND fiscal_year = $2 AND week_number = $3`, [entryData.storeCode, entryData.fiscalYear, entryData.weekNumber - 1]);
            ...
            const retentionRate = previousMembers > 0
                ...
                : 100;
            const previousReviewsResult = await client.query(`SELECT total_google_reviews 
         FROM renoja_weekly_metrics 
         WHERE store_code = $1 AND fiscal_year = $2 AND week_number = $3`, [entryData.storeCode, entryData.fiscalYear, entryData.weekNumber - 1]);
